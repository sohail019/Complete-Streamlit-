...
def setup() -> None:
    ...
    async def capture_image() -> None:
        ...
        if frame is not None:
            ...
            captured_image_ui.set_source(f'data:image/jpeg;base64,{base64.b64encode(jpeg).decode()}')
    ...
    with ui.row().classes('w-full h-full'):
        ...
        video_image = ui.interactive_image().classes('w-1/2 h-full')  # Decreased width to half
        # A timer constantly updates the source of the image.
        # Because data from same paths is cached by the browser,
        # we must force an update by adding the current timestamp to the source.
        ui.timer(interval=0.1, callback=lambda: video_image.set_source(f'/video/frame?{time.time()}'))

        ui.button('Capture Image', on_click=capture_image).classes('m-2')
        captured_image_ui = ui.image().classes('w-1/2 h-full')  # Decreased width to half
